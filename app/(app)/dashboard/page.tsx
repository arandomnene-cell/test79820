    ...
    <main className="bg-white p-6 lg:p-8 min-h-screen">
      <div className="max-w-7xl mx-auto flex flex-col gap-8 lg:gap-10">
        ...
        {activeEnrollment ? (
          ...
          <div className="w-full p-10 bg-slate-50 rounded-3xl border border-slate-100 text-center flex flex-col items-center justify-center min-h-[260px]">
        ...
        {/* Bottom Content: Courses & Stats */}
        <div className="flex flex-col lg:flex-row items-start gap-8 lg:gap-10">
          
          {/* Left Column: My Courses */}
          <div className="flex-1 w-full min-w-0">
            {/* Calculated Height Alignment:
                - Stats Card Height ~ 154px (including padding/borders/gaps)
                - 3 Cards * 154px = 462px
                - 2 Gaps (16px each) = 32px
                - Total = 494px
            ...
            */}
            <section className="bg-slate-50 rounded-3xl border border-slate-200/60 p-6 shadow-sm flex flex-col h-auto lg:h-[30.63rem]">
              ...
              <div className="overflow-y-auto space-y-4 pr-2 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent flex-1">
                {sortedCourses.length > 0 ? (
                  sortedCourses.map((item) => (
                    <Link
                      key={item.id}
                      href={`/courses/${item.course.slug}`}
                      className="group flex gap-4 p-3 bg-white border border-slate-100 rounded-2xl hover:border-eduBlue/40 hover:shadow-md transition-all duration-200 relative overflow-hidden shrink-0"
                    >
                      {item.isFavorite && (
